...
public class Vectors2Topics {
	...
	static CommandOption.Boolean sentiprior = new CommandOption.Boolean
			(Vectors2Topics.class, "use-sentiment-prior", "true|false", false, false,
			 "Rather than assigning topics randomly, assign words to topics properly", null);
	...
	public static void main (String[] args) throws java.io.IOException
	{
		...
		if (usePAM.value) {
			...
			tng.estimate (ilist, numIterations.value, showTopicsInterval.value,
						  outputModelInterval.value, outputModelFilename.value, 
						  randomSeed.value == 0 ? new Randoms() : new Randoms(randomSeed.value),sentiprior.value);
			tng.printTopWords(topWords.value, true, null);
			if (stateFile.value != null)
			...
			if (topicKeysFile.value != null) {
				tng.printTopWords(new File(topicKeysFile.value), topWords.value, false);
			}
			if (outputModelFilename.value != null) {
				assert (tng != null);
				try {
			...
			if (inputModelFilename.value != null) {
				...
				*/
				...
				if(sentiprior.value){					
					topicModel.addInstances(training,true);
				}else{
					topicModel.addInstances(training,false);
				}
			}
