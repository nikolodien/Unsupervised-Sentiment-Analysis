...
import org.iitb.mtp.sair.poc.ir.pipe.ObjectiveWordRemover;
...
public class Text2Vectors {
	 ...
	 * 
	 *         Added to retain only subjective words in the document
	 * 
	 */
	static CommandOption.Boolean removeObjWords = new CommandOption.Boolean(
			Text2Vectors.class, "remove-objective-words", "[TRUE|FALSE]",
			false, false, "If true, remove objectve words from the text.", null);
	
	/**
	 * To add some objective words to the subjective list. These words are useful
	 * for forming sentences using negation like don't like, don't hate, etc
	 */
	static CommandOption.File extraObjWordsFile = new CommandOption.File(
			Text2Vectors.class,
			"extra-objwords",
			"FILE",
			true,
			null,
			"Read whitespace-separated words from this file, and add them to either\n"
					+ "   the default subjective words list",
			null);
	...
	public static void main(String[] args) throws FileNotFoundException,
		...
		if (usePipeFromVectorsFile.wasInvoked()) {
			...
			if (keepSequenceBigrams.value) {
				...
				tokenPattern = CharSequenceLexer.LEX_NEW_WORD_CLASSES;
			} else {
...
//			}
			...
			if (removeObjWords.value) {
				ObjectiveWordRemover objectiveFilter = new ObjectiveWordRemover(
						false, keepSequenceBigrams.value);
				
				if(extraObjWordsFile.wasInvoked()){
					objectiveFilter.addObjWords(extraObjWordsFile.value);
				}
				
				pipeList.add(objectiveFilter);
